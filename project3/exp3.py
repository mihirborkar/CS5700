...
def get_throughput(tvar, qvar, granularity=0.5):
...
def get_drop_rate(tvar, qvar, granularity=0.5):
    pass


def get_latency(tvar, qvar, granularity=0.5):
    f = open(tvar + "-" + qvar + "_output.tr")
...
for tvar in TCP_Variant3:
    for qvar in QUEUE_Variant:
        get_throughput(tvar, qvar)
        get_latency(tvar, qvar)
